...
var _notifyNewConfig = function (user_id, new_config_content, ref){
    return new AV.Promise(function(resolve, reject){
        if(user_id && new_config_content){
            resolve(new Date().toString());
        }else{
            reject("generate datetime error!");
        }
    })
    .then(
        function (updated_at){
            return fb.setUserConfiguration(user_id, new_config_content, updated_at, ref);
        },
        function (error){
...
exports.notifyNewDetails = function(user_id, type, new_detail, source, timestamp, ref){
    return dao.updateUserDetails(user_id, new_detail, type, source, timestamp).then(
        function (result_in_db){
            return AV.Promise.as(
                result_in_db.id,
                result_in_db.get("content"),
                result_in_db.get("timestamp"),
                result_in_db.updatedAt.toString()
            );
        },
        function (error){
        ...
        function (result_id, new_detail, timestamp, updated_at){
          return fb.setUserDetails(user_id, type, new_detail, result_id, timestamp, updated_at, ref);
        },
        function (error){
            return AV.Promise.error(error);
        }
