...
exports.setUserConfiguration = function (user_id, content, updated_at, ref){
    var data = {
    ...
    promises.push(
        object_id_field.set(result_id, function(error){
            if(error){
                ...
                var msg = "User " + user_id + " " + result_id + " is updated successfully!";
                return AV.Promise.as(msg);
            }
        })
    );
    ...
    promises.push(
        update_at_field.set(updated_at, function(error){
            if(error){
                ...
                var msg = "User " + user_id + " " + updated_at + " is updated successfully!";
                return AV.Promise.as(msg);
            } 
        })     
    );
    ...
    promises.push(
        dst_field.set(data, function(error){
            if(error){
                ...
                var msg = "User " + user_id +  " " + type + "  is updated successfully!";
                return AV.Promise.as(msg);
            } 
        })
    );
